...
- ⚠️ markdown **전체 교체**는 맞바꾸기 발생: Q6 신규 실패 + MRR 하락(0.833→0.717)
- ⭐ **[갱신] hybrid 로더(표 페이지만 교체, PR #39) + rewrite가 트레이드오프 없이 해결:**
  **Hit@3 1.000 (실패 0)** — Q18을 잡으면서 Q6·Q10 유지. MRR 0.812(현행 0.833 대비 소폭 하락)
- 판단: 26문항 기준 hybrid+rewrite가 Hit 만점으로 최유력. 단 MRR 소폭 하락과
  66문항 확대 검증이 남아 **최종 채택은 final66 재실험 후** (reranker와의 조합도 후보)
...
| **hybrid(표만) + rewrite+RRF** | **1.000** | 0.812 | **없음** |

※ hybrid 행은 PR #39(표 검출 페이지만 마크다운 교체) merge 후 추가 실험
(`retrieval_es26_rrf_hybrid.csv`). Q18 `[234, 136, 133]` 3위 hit, Q6 `[156, 150, 234]`·
Q10 `[344, 188, 8]` 유지 — 전체 교체의 부작용 없이 표 문항만 개선.
...
- **[갱신] 26문항 최유력: bge-m3 + hybrid 로더 + rewrite+RRF + 페이지 dedup + top_k 3** (Hit@3 1.000)
  — 단 MRR 소폭 하락(0.833→0.812) 있어 최종 채택은 final66 재실험으로 확정
